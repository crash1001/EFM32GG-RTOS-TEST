...
#if configENABLE_DMA_DEBUG == 0
void *pvPortMalloc( size_t xWantedSize )
#else
void *pvPortMallocDebug( size_t xWantedSize, unsigned int line_no, char * filename )
#endif
{
...
void *pvReturn = NULL;
	...
	#if( configUSE_MALLOC_FAILED_HOOK == 1 )
	{
		if( pvReturn == NULL )
		{
			...
			#else
			extern void vApplicationMallocFailedHook_locator( size_t xWantedSize , unsigned int line_no, char * filename);
			vApplicationMallocFailedHook_locator(xWantedSize, line_no, filename);
			#endif
		} else {
			#if configENABLE_DMA_DEBUG == 1
			extern void malloc_call_alert( size_t xWantedSize , unsigned int line_no, char * filename);
			if(xWantedSize > 2000 || xFreeBytesRemaining < 6000) {
				malloc_call_alert(xWantedSize, line_no, filename);
			}
			#endif
		}
	}
...
unsigned char *puc;
	...
	{
		//TRACE("I:%x != B:%x", pxIterator, pxBlockToInsert);
