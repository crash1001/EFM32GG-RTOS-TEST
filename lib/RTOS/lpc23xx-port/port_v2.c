...
/*-----------------------------------------------------------*/
	 ...
	 */
	  ...
	  void vPreemptiveTick( void ) __irq;
...
static void prvSetupTimerInterrupt( void )
{
	...
	LPC_TIM0->TCR = 0x02;
		...
		irq_setup_peripheral(TIMER0_IRQn, vPreemptiveTick, ~0ul);
	#else
		irq_setup_peripheral(TIMER0_IRQn, vNonPreemptiveTick, ~0ul);
	#endif
	/* A 1ms tick does not require the use of the timer prescale.  This is
	defaulted to zero but can be used if necessary. */
	LPC_TIM0->PR = portPRESCALE_VALUE;
	ulCompareMatch = (LPC_SC->PCLKSEL0 >> 2) & 0x03;
	if(ulCompareMatch == 0) {
		ulCompareMatch = 4;
	} else if(ulCompareMatch == 1) {
		ulCompareMatch = 1;
	} else if(ulCompareMatch == 2) {
		ulCompareMatch = 2;
	} else {
		ulCompareMatch = 8;
	}
	ulCompareMatch = configCPU_CLOCK_HZ/ulCompareMatch;

	/* Calculate the match value required for our wanted tick rate. */
	ulCompareMatch = ulCompareMatch / configTICK_RATE_HZ;
	...
	LPC_TIM0->MCR = portRESET_COUNT_ON_MATCH | portINTERRUPT_ON_MATCH;
	LPC_TIM0->MR0 = ulCompareMatch;
...
int
timeout(unsigned int *t, int _msec) {
	portTickType cur_tick = xTaskGetTickCount();
	unsigned int diff;
	int tout = 0;

	diff = cur_tick - *t;
	if(_msec == 0 || diff >= _msec) {
		*t = cur_tick;
		tout = 1;
	}
	return tout;
