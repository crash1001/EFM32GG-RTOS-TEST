...
static void prvSetupTimerInterrupt( void )
{
	extern void irq_install(unsigned int const int_number, void (*handler_addr)(void) __irq);
	...
	T0TCR = 0x02;
		...
		irq_install(4,vPreemptiveTick);
	#else
		irq_install(TIMER0_INT,vNonPreemptiveTick);
	#endif
